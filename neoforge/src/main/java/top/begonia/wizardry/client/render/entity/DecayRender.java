...
import top.begonia.wizardry.client.render.entity.state.DecayRenderState;
...
public class DecayRender extends EntityRenderer<DecayEntity, DecayRenderState> {
    ...
    public void submit(@NonNull DecayRenderState state, @NonNull PoseStack poseStack, @NonNull SubmitNodeCollector submitNodeCollector, @NonNull CameraRenderState camera) {
        ...
        submitNodeCollector.submitCustomGeometry(poseStack, RenderTypes.entityCutout(texture), (pose, vertexConsumer) -> {
            ...
            vertexConsumer.addVertex(pose, 0.0F - f7, 0.0F - f8, -0.01F)
                    .setUv(0, 1)
                    .setColor(255, 255, 255, 255)
                    .setLight(state.lightCoords)
                    .setOverlay(OverlayTexture.NO_OVERLAY)
                    .setNormal(0.0F, 0.0F, 1.0F);
            vertexConsumer.addVertex(pose, f6 - f7, 0.0F - f8, -0.01F)
                    .setUv(1, 1)
                    .setColor(255, 255, 255, 255)
                    .setLight(state.lightCoords)
                    .setOverlay(OverlayTexture.NO_OVERLAY)
                    .setNormal(0.0F, 0.0F, 1.0F);
            vertexConsumer.addVertex(pose, f6 - f7, 1.0F - f8, -0.01F)
                    .setUv(1, 0)
                    .setColor(255, 255, 255, 255)
                    .setLight(state.lightCoords)
                    .setOverlay(OverlayTexture.NO_OVERLAY)
                    .setNormal(0.0F, 0.0F, 1.0F);
            vertexConsumer.addVertex(pose, 0.0F - f7, 1.0F - f8, -0.01F)
                    .setUv(0, 0)
                    .setColor(255, 255, 255, 255)
                    .setLight(state.lightCoords)
                    .setOverlay(OverlayTexture.NO_OVERLAY)
