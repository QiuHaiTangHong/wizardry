package top.begonia.wizardry.client.renderer.entity.block;
...
import top.begonia.wizardry.client.renderer.entity.state.block.LecternRenderState;
...
public class LecternRender implements BlockEntityRenderer<LecternBlockEntity, LecternRenderState> {
    ...
    public void extractRenderState(
        ...
        if (blockEntity.getLevel() != null && blockEntity.getLevel().getBlockState(blockEntity.getBlockPos()).hasProperty(HorizontalDirectionalBlock.FACING)) {
            BlockState blockState = blockEntity.getLevel().getBlockState(blockEntity.getBlockPos());
            state.angle = 90.0F - blockState.getValue(HorizontalDirectionalBlock.FACING).toYRot();
        } else {
            return;
        }
