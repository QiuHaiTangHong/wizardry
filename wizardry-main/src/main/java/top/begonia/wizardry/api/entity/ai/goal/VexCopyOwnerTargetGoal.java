package top.begonia.wizardry.api.entity.ai.goal;

import net.minecraft.world.entity.LivingEntity;
import net.minecraft.world.entity.PathfinderMob;
import net.minecraft.world.entity.Targeting;
import net.minecraft.world.entity.ai.attributes.Attributes;
import net.minecraft.world.entity.ai.goal.Goal;
import net.minecraft.world.entity.ai.targeting.TargetingConditions;
import net.minecraft.world.level.pathfinder.Node;
import net.minecraft.world.level.pathfinder.Path;
import net.minecraft.world.scores.Team;
import org.jspecify.annotations.Nullable;
import top.begonia.wizardry.api.entity.hybrid.ISummonedCreature;

public class VexCopyOwnerTargetGoal<T extends PathfinderMob & ISummonedCreature> extends Goal {
    private static final int EMPTY_REACH_CACHE = 0;
    private static final int CAN_REACH_CACHE = 1;
    private static final int CANT_REACH_CACHE = 2;
    protected final T mob;
    protected final boolean mustSee;
    private final boolean mustReach;
    private final TargetingConditions copyOwnerTargeting;
    protected @Nullable LivingEntity targetMob;
    protected int unseenMemoryTicks;
    private int reachCache;
    private int reachCacheTime;
    private int unseenTicks;

    public VexCopyOwnerTargetGoal(T mob) {
        this(mob, false);
    }

    public VexCopyOwnerTargetGoal(T mob, boolean mustSee) {
        this(mob, mustSee, false);
    }

    public VexCopyOwnerTargetGoal(T mob, boolean mustSee, boolean mustReach) {
        super();
        this.unseenMemoryTicks = 60;
        this.mob = mob;
        this.mustSee = mustSee;
        this.mustReach = mustReach;
        this.copyOwnerTargeting = TargetingConditions.forNonCombat().ignoreLineOfSight().ignoreInvisibilityTesting();
    }

    @Override
    public boolean canUse() {
        if (this.mob.getOwner() instanceof Targeting owner) {
            return owner.getTarget() != null
                    && this.canAttack(
                    owner.getTarget(),
                    this.copyOwnerTargeting
            );
        }

        return false;
    }

    @Override
    public boolean canContinueToUse() {
        LivingEntity target = this.mob.getTarget();
        if (target == null) {
            target = this.targetMob;
        }

        if (target == null) {
            return false;
        } else if (!this.mob.canAttack(target)) {
            return false;
        } else {
            Team mobTeam = this.mob.getTeam();
            Team targetTeam = target.getTeam();
            if (mobTeam != null && targetTeam == mobTeam) {
                return false;
            } else {
                double within = this.getFollowDistance();
                if (this.mob.distanceToSqr(target) > within * within) {
                    return false;
                } else {
                    if (this.mustSee) {
                        if (this.mob.getSensing().hasLineOfSight(target)) {
                            this.unseenTicks = EMPTY_REACH_CACHE;
                        } else if (++this.unseenTicks > reducedTickDelay(this.unseenMemoryTicks)) {
                            return false;
                        }
                    }

                    this.mob.setTarget(target);
                    return true;
                }
            }
        }
    }

    @Override
    public void start() {
        this.reachCache = EMPTY_REACH_CACHE;
        this.reachCacheTime = EMPTY_REACH_CACHE;
        this.unseenTicks = EMPTY_REACH_CACHE;
        LivingEntity target = null;
        if (this.mob.getOwner() instanceof Targeting owner) {
            target = owner.getTarget();
        }
        this.mob.setTarget(target);
    }

    @Override
    public void stop() {
        this.mob.setTarget(null);
        this.targetMob = null;
    }

    protected double getFollowDistance() {
        return this.mob.getAttributeValue(Attributes.FOLLOW_RANGE);
    }

    protected boolean canAttack(@Nullable LivingEntity target, TargetingConditions targetConditions) {
        if (target == null) {
            return false;
        } else if (!targetConditions.test(getServerLevel(this.mob), this.mob, target)) {
            return false;
        } else if (!this.mob.isWithinHome(target.blockPosition())) {
            return false;
        } else {
            if (this.mustReach) {
                if (--this.reachCacheTime <= EMPTY_REACH_CACHE) {
                    this.reachCache = EMPTY_REACH_CACHE;
                }

                if (this.reachCache == EMPTY_REACH_CACHE) {
                    this.reachCache = this.canReach(target) ? CAN_REACH_CACHE : CANT_REACH_CACHE;
                }

                return this.reachCache != CANT_REACH_CACHE;
            }

            return true;
        }
    }

    private boolean canReach(LivingEntity target) {
        this.reachCacheTime = reducedTickDelay(10 + this.mob.getRandom().nextInt(5));
        Path path = this.mob.getNavigation().createPath(target, EMPTY_REACH_CACHE);
        if (path == null) {
            return false;
        } else {
            Node last = path.getEndNode();
            if (last == null) {
                return false;
            } else {
                int xx = last.x - target.getBlockX();
                int zz = last.z - target.getBlockZ();
                return (double) (xx * xx + zz * zz) <= (double) 2.25F;
            }
        }
    }

    public VexCopyOwnerTargetGoal<T> setUnseenMemoryTicks(int unseenMemoryTicks) {
        this.unseenMemoryTicks = unseenMemoryTicks;
        return this;
    }
}
