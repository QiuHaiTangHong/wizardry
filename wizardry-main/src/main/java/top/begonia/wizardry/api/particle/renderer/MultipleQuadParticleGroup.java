...
public class MultipleQuadParticleGroup extends ParticleGroup<WizardryQuadParticle> {
    private final MultipleQuadParticleRenderState particleTypeRenderState = new MultipleQuadParticleRenderState();

    public MultipleQuadParticleGroup(ParticleEngine engine) {
        super(engine);
    }
    ...
    public @NonNull ParticleGroupRenderState extractRenderState(@NonNull Frustum frustum, @NonNull Camera camera, float partialTickTime) {
        for (WizardryQuadParticle particle : this.particles) {
